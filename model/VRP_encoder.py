...
class VRP_encoder(nn.Module):
    def __init__(self, args, backbone, use_original_imgsize):
        ...
        self.transformer_decoder = transformer_decoder(args, args.num_query, hidden_dim, hidden_dim*2, num_layers=args.num_layers)

        self.cross_entropy_loss = nn.CrossEntropyLoss()
        self.mse_loss = nn.MSELoss()
        self.bce_with_logits_loss = nn.BCEWithLogitsLoss()

    def forward(self, condition, query_sam_feat, supp_sam_feat, query_img, query_mask, support_img, support_mask, training):
        ...
        with torch.no_grad():
            ...
            if self.backbone_type == 'vgg16':
                query_feat_2 = F.interpolate(query_feat_2, size=(64,64), mode='bilinear', align_corners=True)
                query_feat_3 = F.interpolate(query_feat_3, size=(64,64), mode='bilinear', align_corners=True)
                query_feat_4 = F.interpolate(query_feat_4, size=(64,64), mode='bilinear', align_corners=True)
            ...
            _supp_feat_4 = self.layer4(supp_feat_3 * support_mask)
        ...
        prototype_sam = self.mask_feature(supp_sam_feat, support_mask)
        supp_feat_bin_sam = prototype_sam.repeat(1, 1, query_feat.shape[2], query_feat.shape[3])

        supp_sam_feat = self.downsample_sam_query(torch.cat([supp_sam_feat], 1))
        query_sam_feat = self.downsample_sam_query(torch.cat([query_sam_feat], 1))

        """feature projection for pseudo mask"""
        spt_prototype = self.mask_feature(supp_feat_4, support_mask)

        """generating prompts for sam mask decoder"""
        protos, attn_lst, dict_for_loss = self.transformer_decoder(query_feat, supp_feat, query_sam_feat, supp_sam_feat, support_mask, spt_prototype, query_feat_4, pseudo_mask, supp_feat_bin, supp_feat_bin_sam)

        return protos, attn_lst, dict_for_loss
    ...
    def predict_mask_nshot(self, args, batch, sam_model, nshot, input_point=None):
        ...
        for s_idx in range(nshot):
            ...
            protos_sub, support_mask, _ = self(args.condition, query_sam_feat, supp_sam_feat, batch['query_img'], batch['query_mask'],  batch['support_imgs'][:, s_idx], batch['support_masks'][:, s_idx], False)
